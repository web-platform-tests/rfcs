...
fire the `browser.test.onTestStarted` and `browser.test.onTestFinished` event listeners, respectively.
...
* `listener` (function: (message: string, data: object) => void) – A function that gets called when a test has started.
...
* `listener` (function: (message: string, data: object) => void) – A function that gets called when a test has finshed.
